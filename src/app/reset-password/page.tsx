...
import { actionUpdatePassword } from "@/lib/server-action/auth-actions";
...
export default function ResetPassword() {
  ...
  > = async (formData) => {
    const { error } = await actionUpdatePassword(formData.password);
    if (error) {
      form.reset();
            ...
            >
              ...
              {(confirmation || codeExchangeError) && (
                <>
                  ...
                  >
                    {!codeExchangeError && <Check className="h-4 w-4" />}
                    <AlertTitle>
                      {codeExchangeError ? "Invalid Link" : "Check your email."}
                    </AlertTitle>
                    <AlertDescription>
                      {codeExchangeError || "Password has been changed."}
                    </AlertDescription>
                  </Alert>
                </>
              )}
